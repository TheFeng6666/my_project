...
uint16_t FSUS_Servo::querymonitor()
{
    ...
    [[maybe_unused]] FSUS_STATUS status = this->protocol->recvQueryServoMonitor(&this->servoId, &this->servomonitor);
    return this->servomonitor.status;
}
...
/* 设置舵机同步的原始角度 */
void FSUS_Servo::setSyncRawAngle(FSUS_SERVO_COUNT_T servo_count, [[maybe_unused]] FSUS_Sync_T Syncsend[])
{
    this->protocol->sendSyncCommand(servo_count, MODE_SET_SERVO_ANGLE, Syncsend);
}
// 设置舵机同步的原始角度(指定周期)
void FSUS_Servo::setSyncRawAngleByInterval(FSUS_SERVO_COUNT_T servo_count, [[maybe_unused]] FSUS_Sync_T Syncsend[])
{
    this->protocol->sendSyncCommand(servo_count, MODE_SET_SERVO_ANGLE_BY_INTERVAL, Syncsend);
}
// 设定舵机同步的原始角度(指定转速)
void FSUS_Servo::setSyncRawAngleByVelocity(FSUS_SERVO_COUNT_T servo_count, [[maybe_unused]] FSUS_Sync_T Syncsend[])
{
    this->protocol->sendSyncCommand(servo_count, MODE_SET_SERVO_ANGLE_BY_VELOCITY, Syncsend);
}
// 设定舵机同步的原始角度(多圈)
void FSUS_Servo::setSyncRawAngleMTurn(FSUS_SERVO_COUNT_T servo_count, [[maybe_unused]] FSUS_Sync_T Syncsend[])
{
    this->protocol->sendSyncCommand(servo_count, MODE_SET_SERVO_ANGLE_MTURN, Syncsend);
}
// 设定舵机同步的原始角度(多圈+指定周期)
void FSUS_Servo::setSyncRawAngleMTurnByInterval(FSUS_SERVO_COUNT_T servo_count, [[maybe_unused]] FSUS_Sync_T Syncsend[])
{
    this->protocol->sendSyncCommand(servo_count, MODE_SET_SERVO_ANGLE_MTURN_BY_INTERVAL, Syncsend);
}
// 设定舵机同步的原始角度(多圈+指定转速)
void FSUS_Servo::setSyncRawAngleMTurnByVelocity(FSUS_SERVO_COUNT_T servo_count, [[maybe_unused]] FSUS_Sync_T Syncsend[])
{
    this->protocol->sendSyncCommand(servo_count, MODE_SET_SERVO_ANGLE_MTURN_BY_VELOCITY, Syncsend);
}
//设定舵机同步的数据监控
